...
def generator(possibility: float, size, q, r: int) -> set:
    _edges = set()
...
def converter(_edges: set, size: int) -> list:
...
def dijkstra(_adj_matrix: list, start, size: int) -> [list, list]:
    int_max = 10**10
...
def ford_bellman(_edges: set, start, size: int) -> [list, list]:
    int_max = 10**10
    distance = [int_max] * size
    distance[start] = 0
    last_visited = [-1] * size
